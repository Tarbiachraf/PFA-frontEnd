...
const ModefieFacture = ()=>{
    ...
    const [formData, setFormData] = useState({
      ...
      const handleUpdateFacture = (event) => {
          ...
          FactureService.updateFacture(formData, idFacture)
              .then(response => {
                  setAlert({ type: 'success', message: 'Facture updated successfully!' });
              })
              .catch(error => {
                  console.error('Error updating facture:', error);
                  setAlert({ type: 'error', message: 'Failed to update facture.' });
              });
      }
